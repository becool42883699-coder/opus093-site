...
1. **面（背景）は2色だけ。** `--bc-white` と `--bc-paper` のみ。セクションごとに
   `.surfaceWhite` / `.surfacePaper` を**明示して交互に**敷く（白→紙→白→紙）。
   コンポーネント側に背景を持たせない（第3の明色が生えるため）。旧 `--bc-works-bg`
   （#e7ecf2）は paper のエイリアスにしてあるので新規に使わない。
