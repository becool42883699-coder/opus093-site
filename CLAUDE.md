...
- 写真は **AI生成の画像を `public/images/` に置いてある**（2026-08-21〜）。
  出どころ・寸法・書き出しの決まり・配置済みの枚数は `public/images/CREDITS.md`。
  実在の建物ではない。読み込みに失敗したら灰白の下地にパス名を出す
  フォールバックは**残してある**（1枚欠けても壊れない）。`Shot` のこの2段構え
  （`onError` ＋ マウント時の `complete && naturalWidth === 0`）を消さないこと（§3-17）。
- **素の `<img src>` に basePath は付かない。** Next が書き換えるのは自前アセットと
  next/image だけなので、`/images/…` のままだと公開URLで必ず404になる。
  ローカルでは通るのでデプロイして初めて壊れる。`Shot` の中で
  `process.env.NEXT_PUBLIC_BASE_PATH` を前置きして解決している。
- **`<img>` には `draggable={false}` が要る。** 既定のネイティブドラッグが
  カルーセルの framer-motion のドラッグを奪い、スワイプが効かなくなる
  （プレースホルダは `<span>` だったので写真を入れるまで表面化しなかった）。
- ヒーローは画面幅で `4:5` / `4:3` / `3:4` の**3通りに切られる**。差し替える時は
  主題を中央に置き四辺に余白を残し、3幅とも実機幅で確認する。
