...
| DELETE | `/api/projects/:slug?confirm=<slug>` | 同上 | 案件削除(全版のファイル+メタ情報) |
...
| 案件削除 | 実装済み。誤操作防止のため `?confirm=<slug>` 必須(UIはスラッグの再入力を要求)。R2は1000件ずつ削除し、1リクエストで最大20000ファイルまで(超える場合は再実行を促す)。R2→KVの順に消すため、途中失敗しても案件は残り再実行できる |
