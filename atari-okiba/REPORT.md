...
| 7 | 表示名変更・閲覧パスワードの後から変更(PATCH)に加え、案件削除(DELETE)も実装 | 個人ハブとして案件が溜まり続けるのを避けるため。誤操作防止にスラッグ再入力+`?confirm=`を要求 |
...
| 2. vitest(@cloudflare/vitest-pool-workers)で完了条件を自動テスト | ✅ PASS | 52テスト全パス。実workerd上でWorker全体(SELF)を叩き、完了条件1〜5+セキュリティ要件+案件削除を網羅 |
...
### 案件削除の実装中に自分で作り込み、実機検証で発見・修正したバグ

管理画面のJSはTypeScriptのテンプレートリテラル内に置いているため、`\n` のようなエスケープを
書くと**出力HTMLのJS文字列が実際の改行で分断され構文エラーになる**(管理画面のJSが丸ごと停止し、
ログインフォームがネイティブ送信されてトークンがURLに載る)。削除ボタンの確認メッセージで
これを踏み、Playwright実機検証で発見して修正した。同種の再発を防ぐため、

- 出力HTMLのインラインスクリプトに「行内で閉じていない文字列リテラル」がないことを検証するテストを追加
  (バグを意図的に戻すとFAILすることを確認済み)
- トークン入力欄から `name` 属性を削除し、万一JSが停止してネイティブ送信されても
  クエリ文字列にトークンが乗らないようにした(パスワードマネージャは `autocomplete` で機能する)
